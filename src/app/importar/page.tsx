    ...
    <section className="space-y-4">
      ...
      <section className="space-y-3 rounded-2xl border border-ink/10 bg-white p-4 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <button type="button" onClick={loadMetadata} className="rounded-lg border border-ink/20 px-3 py-1 text-sm">
            Recarregar metadata
          </button>
        </div>
      ...
      <section className="space-y-3 rounded-2xl border border-ink/10 bg-white p-4 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={doPreview} className="rounded-lg bg-ink px-4 py-2 text-sand">
            Gerar preview
          </button>
          <button type="button" onClick={runImport} className="rounded-lg bg-pine px-4 py-2 text-white">
            Executar importacao
          </button>
        </div>
        ...
        {preview ? (
          <div className="space-y-3 rounded-lg bg-sand p-3 text-sm">
            ...
            {preview.onlyNegative ? (
              <p className="text-ink/70">Filtro ativo: somente negativos.</p>
            ) : null}
            ...
            {preview.monthPreviews.map((monthItem) => (
              <div key={`${monthItem.month}-${monthItem.startCol}`} className="rounded border border-ink/15 bg-white p-3">
                ...
                {preview.onlyNegative ? (
                  <p className="text-ink/70">Modo ajustes: nao pula meses ja importados.</p>
                ) : null}
